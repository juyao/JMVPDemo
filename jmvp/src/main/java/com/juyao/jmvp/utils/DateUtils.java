...
public class DateUtils {
    ...
    public static String todayYear(){
        return todayYyyyMmDd().split("-")[0];
    }
    /**
     * 当天的月
     */
    public static String todayMonth(){
        return todayYyyyMmDd().split("-")[1];
    }
    /**
     * 当天的日
     */
    public static String todayDay(){
        return todayYyyyMmDd().split("-")[2];
    }
    /**
     * Long转Date
     */
     ...
     * @return
     */
    public static String formatYyyyMmDd(String dateTime) {
        String result = "";

        try {
            Date e = YYYYMMDD_FORMAT.parse(dateTime);
            result = YYYYMMDD_FORMAT.format(e);
        } catch (ParseException var3) {
            var3.printStackTrace();
        }
     ...
     * 时分
     */
    public static String parseHhMm(String time){
        return HHMM_FORMAT.format(parseTime(time));
    }
    ...
    public static boolean dateIsBefore(String standDate, String desDate) {
        try {
            return YYYYMMDD_FORMAT.parse(desDate).before(YYYYMMDD_FORMAT.parse(standDate));
        } catch (ParseException var3) {
            var3.printStackTrace();
            return false;
        }
...
//    }
     ...
     * 获取当前日期增加n天后的日期
     */
    public static String getDataAfterDay(int n){
        Calendar calendar = Calendar.getInstance();
     ...
     * 获取当前日期增加n天后的日期
     */
    public static String getDataAfterDay_simple(int n){
        Calendar calendar = Calendar.getInstance();
        calendar.add(Calendar.DAY_OF_MONTH, n);
        int year = calendar.get(Calendar.YEAR);
        int month = calendar.get(Calendar.MONTH)+1;
